...
def read_one(lname):
    ...
    :param lname:   last name of people to find
    :return:        person matching last name
    """
    if lname in PEOPLE:
        person = PEOPLE.get(lname)
    else:
        abort(404, 'People with last name {lname} not found.'.format(
            lname=lname))
    return person
...
def create(person):
    ...
    if lname not in PEOPLE and lname is not None:
        ...
        abort(406, 'Person with last name {lname} already exists'.format(
            lname=lname))
